...
Lightning payments are also highly applicable to the IoT space, as the network can be used used to implement a decentralized peer-to-peer payment layer for transactions between IoT devices, utilizing all of the networks key features, see [IoT and Lightning](https://medium.com/meetbitfury/the-internet-of-things-and-the-lightning-network-41b93dbb8456). For cryptocurrency exchanges,
...
In order to receive payment for a service, an invoice should be generated by the payee and sent to the payer.
...
In the example below, a dictionary containing three key-value pairs is passed as input to the API.
...
Below is an example of an invoice generated by a Lightning-enabled eCommerce website.
...
Users can test the API by buying small items on the various Lighting-enabled stores. 
Below is an example of a payment request generated by Blockstream store, presented a checkout time.
